	...
	try {
		...
		if (netUsdcMicros < config.minNetUsdcMicros) {
			return {
				...
				errorMessage:
					"Refund net amount is below AUTO_REFUND_MIN_NET_USDC_MICROS",
				grossUsdcMicros,
				gasFeeUsdcMicros,
				netUsdcMicros,
				estimate,
...
async function estimateRefundGasFeeMicros(input: {
	...
	const config = getAutoRefundConfig();
	const gasCostUsdMicros = decimalToMicros(
		Number(formatEther(gasCostWei)) * polUsd,
	);
...
async function sendPusdRefund(input: { recipient: string; amount: bigint }) {
	...
	if (getAddress(account.address) !== feeWallet) {
		throw new Error(
			"PLATFORM_FEE_PRIVATE_KEY does not match PLATFORM_FEE_WALLET",
		);
	}
